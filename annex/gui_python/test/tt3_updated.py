# Полный обновленный tt3.py с поддержкой:
# - задания индексов переменных вручную (var_mask)
# - выравнивания по центру
# - запрета редактирования таблицы при просмотре
# - безопасного переключения режимов
...
class PixelLogicViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.types_name = {
            0: "Результат Комб.", 1: "Комбинаторная логика", 2: "Результат Послед.",
            3: "Последовательностная логика", 4: "Результат автомат.", 5: "Теория автоматов"
        }
    ...
    def init_ui(self):
        ...
        for lbl in [self.label_id, self.label_type, self.label_speed, self.task_text]:
    ...
    def enter_setup_mode(self):
        ...
        input_group = QGroupBox("CREATE NEW TASK")
        ...
        form_layout.addWidget(QLabel("Variable indexes:"))
        form_layout.addWidget(self.new_file_data["var_indices"])
        ...
        if self.setup_mode:
            ...
            if existing_func and existing_func.num_vars == num_vars:
                for i in range(num_rows):
                    ...
                    bit_data[i] = bit
                indices = [str(i) for i in range(16) if (existing_func.var_mask >> i) & 1]
                self.new_file_data["var_indices"].setText(",".join(indices))

            for i in range(num_rows):
                for j in range(num_vars):
                    bit_item = QTableWidgetItem(str((i >> (num_vars - j - 1)) & 1))
    ...
    def rebuild_task_panel(self):
        ...
        self.task_text = QTextEdit()
        self.task_text.setReadOnly(True)
        for w in [self.label_id, self.label_type, self.label_speed, self.task_text]:
    ...
    def update_table(self):
        ...
        for i in range(num_rows):
            for j in range(func.num_vars):
                bit_item = QTableWidgetItem(str((i >> (func.num_vars - j - 1)) & 1))
                bit_item.setTextAlignment(Qt.AlignCenter)
                bit_item.setFlags(Qt.ItemIsEnabled | Qt.ItemIsSelectable)
                self.table.setItem(i, j, bit_item)
            result_bit = (func.bit_vector[i >> 3] >> (i % 8)) & 1
